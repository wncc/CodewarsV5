...
class Dragon(Troop):
    def __init__(self,images , position, surf, myTower, std_size, uid):
        super().__init__(name="Dragon", images  = images, position = position, elixir = 4,
                         health=1267, damage=176, velocity=FAST_SPEED, type_="air", attack_range = 3.5, surf = surf, discovery_range = 6, myTower = myTower,
...
class Minion(Troop):
    def __init__(self, position, images , surf, myTower,std_size, uid):
        super().__init__(name="Minion", images = images, position = position, elixir = 5,
                         health=252, damage=129, velocity=FAST_SPEED, type_="air", attack_range = 2, surf = surf, discovery_range = 6, myTower = myTower,
                         target_type = {"air": True, "ground": True, "building": True}, splash_range = 0, size = 0.15,std_size= std_size, uid = uid, attack_speed = FAST_ATTACK,
                         number=3) 
...
class Musketeer(Troop):
    def __init__(self,images , position, surf, myTower,std_size, uid):
        super().__init__(name="Musketeer", images = images, position = position, elixir = 4,
                         health=792, damage=239, velocity=MEDIUM_SPEED, type_="ground", attack_range = 6, surf = surf, discovery_range = 8, myTower = myTower,
